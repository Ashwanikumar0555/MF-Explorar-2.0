...
//       {currentPage === "list" ? (
//         <FundsListPage onSelectFund={(code:any) => {
...
// function FundsListPage({ onSelectFund }:any) {
...
//   const toggleWatchlist = ({fund}:any) => {
//     setWatchlist(prev => {
...
//   const isInWatchlist = (schemeCode:any) => {
...
//   // Memoize filtered funds to prevent recalculation on every render
...
//     });
//   }, [data, query, categoryFilter, activeTab, watchlist]);

//   const totalPages = Math.ceil(filteredFunds.length / ITEMS_PER_PAGE);
  
//   // Memoize paginated funds
//   const paginatedFunds = useMemo(() => {
//     return filteredFunds.slice(
//       (currentPage - 1) * ITEMS_PER_PAGE,
//       currentPage * ITEMS_PER_PAGE
//     );
//   }, [filteredFunds, currentPage, ITEMS_PER_PAGE]);

//   // Reset displayed funds when page or filters change
//   useEffect(() => {
//     setDisplayedFunds(paginatedFunds.slice(0, 6));
//   }, [currentPage, query, categoryFilter, activeTab]);
...
//             const newItems = paginatedFunds.slice(nextIndex, nextIndex + 6);
...
// function FundDetailsPage({ schemeCode, onBack }:any) {
...
//   const calculateReturns = ({data, days}:any) => {
...
//   const calculateRollingReturns = useMemo(() => {
//     return (period) => {
//       if (!fundDetails?.data) return [];
...
function FundsListPage({ onSelectFund }) {
  ...
  const isInWatchlist = (schemeCode) => {
    return watchlist.some(f => f && f.schemeCode === schemeCode);
  };

  const getCategories = () => {
    const categories = new Set();
    data.forEach(fund => {
      if (fund && fund.schemeName) {
        const match = fund.schemeName.match(/\b(Equity|Debt|Hybrid|Liquid|ELSS|Index|Balanced)\b/i);
        if (match) categories.add(match[0]);
      }
    });
